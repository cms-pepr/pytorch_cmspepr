...
def calc_q_betaclip(beta, qmin=1.):
...
class single:
    ...
    def losses(cls):
        ...
        V_att = d(2,4) * cls.q[2] * q[4] / 5. # Since d is small, d == d_huber
        V_rep = (
            torch.exp(-4.*d(0,4)) * q[0] * q[4]
            + torch.exp(-4.*d(1,4)) * q[1] * q[4]
            + torch.exp(-4.*d(3,4)) * q[3] * q[4]
            ) / 5.
        V_srp = -1./(20.*d(2,4) + 1.) * beta[4] / 2.
        L_beta_cond_logterm = -0.2 * log(beta[4]+1e-9)
        L_beta_noise = (beta[0]+beta[1]+beta[3]) / 3.

        losses_man = torch.FloatTensor([V_att, V_rep, V_srp, L_beta_cond_logterm, L_beta_noise])
        return losses_man
...
def test_oc_cpu_batch():
    ...
    loss_py = objectcondensation.oc_loss(multiple.model_out, Data(y=multiple.y, batch=multiple.batch))
    losses_py = torch.FloatTensor([
        loss_py["V_att"], loss_py["V_rep"],
        loss_py["L_beta_sig"], loss_py["L_beta_cond_logterm"],
